...
TexturedMesh::TexturedMesh(std::vector<TexturedVertex> vertices, std::vector<unsigned int> indices, unsigned int texture) {
  this->vertices = std::move(vertices);
  this->indices = std::move(indices);
...
void TexturedMesh::SetUp() {
  ...
  glEnableVertexAttribArray(0);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(TexturedVertex), (void *) nullptr);
  glEnableVertexAttribArray(1);
